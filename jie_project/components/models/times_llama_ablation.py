...
from ..layers import MLP, MoE


class TimesLlamaAblation(nn.Module):
    def __init__(self, configs):
        ...
        # patching
        # self.extend_encoder = nn.Linear(self.token_len, self.hidden_dim_of_llama)
        self.fusion_gate = nn.Parameter(torch.zeros([])) if configs.mix_embeds else None
        self.extend_encoder = nn.Linear(self.token_len, self.hidden_dim_of_llama)
        self.input_encoder = nn.Linear(self.hidden_dim_of_llama, self.hidden_dim_of_llama)

        # moe
        self.moe = MoE(self.hidden_dim_of_llama, self.hidden_dim_of_llama)
        # self.alter_moe = nn.Linear(self.hidden_dim_of_llama, self.hidden_dim_of_llama)
        ...
        if configs.mlp_hidden_layers == 0:
            ...
            # self.encoder = MLP(self.hidden_dim_of_llama * 2, self.hidden_dim_of_llama,
    ...
    #     x_enc = self.extend_encoder(x_enc)
    #
    #     times_embeds = x_enc
    #     times_embeds = self.input_encoder(times_embeds)
    #
    ...
    #     x_enc = self.extend_encoder(x_enc)
    #
    #     times_embeds = torch.cat([x_mark_enc, x_enc], dim=-1)
    #     times_embeds = self.input_encoder(times_embeds)
    #
    #     outputs = self.llama.model(inputs_embeds=times_embeds)[0]
    #
    #     # MoE
    #     outputs = self.alter_moe(outputs)
    #
    #     dec_out = self.decoder(outputs)
    #     dec_out = dec_out.reshape(bs, 1, -1)[:, :, :self.seq_len]
    #     dec_out = dec_out.permute(0, 2, 1)
