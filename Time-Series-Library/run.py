...
if __name__ == '__main__':
    ...
    parser.add_argument('--kernel_size', type=int, default=None, help='decomposition kernel size for TFDFNet')
    parser.add_argument('--in_channels', type=int, default=None, help='input channel count for TFDFNet; defaults to enc_in')
    parser.add_argument('--out_channels', type=int, default=None, help='output channel count for TFDFNet; defaults to c_out')
    parser.add_argument('--time_feature_dim', type=int, default=4, help='time feature channel count for TFDFNet')
    ...
    # PatchGatedLSTM
    parser.add_argument('--patch_stride', type=int, default=12, help='patch stride for PatchGatedLSTM')
    parser.add_argument('--top_k_patches', type=int, default=8, help='number of selected patches for PatchGatedLSTM')
    parser.add_argument('--hidden_size', type=int, default=64, help='LSTM hidden size for PatchGatedLSTM')
    parser.add_argument('--model_dim', type=int, default=64, help='patch token projection size for PatchGatedLSTM')
    parser.add_argument('--lstm_layers', type=int, default=2, help='number of LSTM layers for PatchGatedLSTM')
    parser.add_argument('--fft_keep_ratio', type=float, default=0.35, help='low-frequency keep ratio for PatchGatedLSTM')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.model == 'MambaSingleLayer' and args.task_name == 'classification':
            ...
            elif args.model == 'PatchGatedLSTM':
                setting = f'{args.task_name}_{args.model_id}_{args.model}_{args.data}_ft{args.features}' \
                        ...
                        + f'_fft{args.fft_keep_ratio}_eb{args.embed}_dt{args.distil}_{args.des}_{ii}'
