...
class QuantileStandardScaler:
    def __init__(self, lower=0.01, upper=0.99, eps=1e-6):
        self.lower = lower
        self.upper = upper
        self.eps = eps
    ...
    def fit(self, data):
        ...
        self.q_low = np.nanquantile(values, self.lower, axis=0)
        self.q_high = np.nanquantile(values, self.upper, axis=0)
        clipped = np.clip(values, self.q_low, self.q_high)
        self.mean = np.nanmean(clipped, axis=0)
        self.std = np.nanstd(clipped, axis=0)
        self.mean = np.nan_to_num(self.mean, nan=0.0, posinf=0.0, neginf=0.0)
        self.std = np.nan_to_num(self.std, nan=1.0, posinf=1.0, neginf=1.0)
        self.std[self.std < self.eps] = 1.0
        return self

    def transform(self, data):
        values = np.asarray(data, dtype=np.float32)
        values = np.clip(values, self.q_low, self.q_high)
...
def _prepare_custom_dataframe(df_raw, target):
    if 'date' not in df_raw.columns and 'timestamp' in df_raw.columns:
        df_raw = df_raw.rename(columns={'timestamp': 'date'})
    if 'date' not in df_raw.columns:
        raise ValueError("custom dataset must contain a 'date' or 'timestamp' column")
    if target not in df_raw.columns:
        raise ValueError(f"custom dataset does not contain target column {target!r}")

    df_raw = df_raw.copy()
    df_raw['date'] = pd.to_datetime(df_raw['date'], errors='coerce')
    df_raw = df_raw.dropna(subset=['date']).sort_values('date').drop_duplicates('date')

    cols = [c for c in df_raw.columns if c not in ['date', target]]
    df_raw = df_raw[['date'] + cols + [target]]
    numeric_cols = cols + [target]
    ...
    return df_raw[['date'] + numeric_cols]


def _prepare_building_dataframe(df_raw, target):
    df_raw = df_raw.copy()
    for column in ['precipDepth1HR', 'precipDepth6HR']:
        if column in df_raw.columns:
            df_raw = df_raw.drop(columns=[column])
    return _prepare_custom_dataframe(df_raw, target)
...
class Dataset_Custom(Dataset):
    ...
    def _prepare_dataframe(self, df_raw):
        return _prepare_custom_dataframe(df_raw, self.target)

    def __read_data__(self):
        self.scaler = QuantileStandardScaler()
        local_fp = os.path.join(self.root_path, self.data_path)
        cfg_name = os.path.splitext(os.path.basename(self.data_path))[0]
        ...
        '''
        df_raw = self._prepare_dataframe(df_raw)
...
class Dataset_Building(Dataset_Custom):
    def _prepare_dataframe(self, df_raw):
        return _prepare_building_dataframe(df_raw, self.target)
