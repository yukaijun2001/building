...
output_root=${OUTPUT_ROOT:-${script_dir}/output/TFDFNet参数}
...
cols = [c for c in header if c not in time_cols and c not in drop_cols]
...
PY
  ...
  for seq_len in ${seq_lens}; do
    for pred_len in ${PRED_LENS:-24}; do
      ...
      "${python_bin}" -u "${script_dir}/measure_params.py" \
        --project_dir "${tsl_dir}" \
